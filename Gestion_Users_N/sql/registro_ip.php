...
<body>
    ...
    if (isset($_POST['register_user_ip'])) {
        ...
        $sql_v = "SELECT COUNT(1), UL.ID_PERMISO, UL.IP_USUARIO FROM PRESTACIONES.USUARIO_LOGIN UL
        WHERE UL.ID_USUARIO = 'A'
        GROUP BY UL.ID_PERMISO, UL.IP_USUARIO";
        ...
        $add_ip_user = $v_permiso['IP_USUARIO'];

        if ($dir_ip_equipo === $add_ip_user) { ?>
            <script>
                Swal.fire({
                        title: "La IP ya Existe en el USUARIO Seleccionado",
                        icon: "error",
